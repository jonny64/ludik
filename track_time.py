...
SETTINGS_FILE = 'ludik.sublime-settings'
SETTINGS = {}
LAST_HEARTBEAT = {
    'time': 0,
    'file': None,
...
def createConfigFile():
    ...
    not already exist.
    """
    configFile = os.path.join(os.path.expanduser('~'), '.ludik.cfg')
    try:
...
def prompt_api_key():
    ...
    if SETTINGS.get('api_key'):
        ...
        if window:
            window.show_input_panel('[ludik] Enter your time track api key:', default_key, got_key, None, None)
            return True
        else:
            print('[ludik] Error: Could not prompt for api key because no window found.')
    return False
...
class SendHeartbeatThread(threading.Thread):
    ...
    def send_heartbeat(self):
        if not self.api_key:
            print('[ludik] Error: missing api key.')
            return
        ua = 'sublime/%d ludik/%s' % (ST_VERSION, __version__)
        cmd = [
            API_CLIENT,
        ...
        if python_binary():
            ...
            if self.debug:
                print('[ludik] %s' % ' '.join(obfuscate_apikey(cmd)))
            ...
            print('[ludik] Error: Unable to find python binary.')
